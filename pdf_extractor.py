...
def image_extractor(file_path):
    ...
    image_output_folder = os.path.join("data", "extracted_images", file_name)
    ...
    try:
        ...
        for page_num in range(len(doc)):
            ...
            for i, block in enumerate(text_blocks):
                ...
                if caption_regex.match(block_text.strip()):
                    ...
                    # 4. Final capture
                    if target_block_bbox:
                        final_capture_bbox = target_block_bbox.include_rect(
                            caption_bbox
                        )
                        ...
                        safe_filename = re.sub(r'[\\/*?:"<>|]', "", block_text.strip())[
                            :40
                        ]
        ...
        print(
            f"[Final Result] Failed to capture a total of {failed_captures} instances."
        )
        doc.close()
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="PDF Figure Extractor")
    parser.add_argument(
        "pdf_file",
        type=str,
